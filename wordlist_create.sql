...
DROP TABLE IF EXISTS up;     -- 좋아요
DROP TABLE IF EXISTS down;   -- 싫어요
DROP TABLE IF EXISTS notes;  -- 댓글
DROP TABLE IF EXISTS deals;  -- 말다듬기
DROP TABLE IF EXISTS exprs;  -- 말(표현)

CREATE TABLE exprs (
  id   INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  expr VARCHAR(200) NOT NULL  -- 말(표현): 다듬을 말, 다듬은 말
);

CREATE TABLE deals (
  id   INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  de   INT UNSIGNED NULL,  -- 다듬을 말
  al   INT UNSIGNED NULL,  -- 다듬은 말
  user INT UNSIGNED NULL,  -- 다듬은 이
  c TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  t TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY(de) REFERENCES exprs(id) ON UPDATE CASCADE,
  FOREIGN KEY(al) REFERENCES exprs(id) ON UPDATE CASCADE,
  FOREIGN KEY(user) REFERENCES users(id) ON UPDATE CASCADE
);

CREATE TABLE up (
  deal INT UNSIGNED NOT NULL,  -- 좋아요 단 말다듬기
  user INT UNSIGNED NOT NULL,  -- 좋아요 단 이
  FOREIGN KEY(deal) REFERENCES deals(id) ON UPDATE CASCADE,
  FOREIGN KEY(user) REFERENCES users(id) ON UPDATE CASCADE
);

CREATE TABLE down (
  deal INT UNSIGNED NOT NULL,  -- 싫어요 단 말다듬기
  user INT UNSIGNED NOT NULL,  -- 싫어요 단 이
  FOREIGN KEY(deal) REFERENCES deals(id) ON UPDATE CASCADE,
  FOREIGN KEY(user) REFERENCES users(id) ON UPDATE CASCADE
);
...
CREATE TABLE notes (
  id   INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  data VARCHAR(15000) NOT NULL,  -- 댓글
  deal INT UNSIGNED NOT NULL,    -- 댓글 단 말다듬기
  user INT UNSIGNED NOT NULL,    -- 댓글 단 이
  t TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY(deal) REFERENCES deals(id) ON UPDATE CASCADE,
  FOREIGN KEY(user) REFERENCES users(id) ON UPDATE CASCADE
);
